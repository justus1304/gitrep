...
import uncertainity as unc
## Zweite Linearverstärkermessung
f_kHz, U_out_values = np.genfromtxt("Daten/L2.txt", unpack=True)
f_Hz = f_kHz * 1000

U_e = ufloat(1.01, 0.02)
U_out = unumpy.uarray(U_out_values, 0.1)
V = U_out/U_e

# Leerlaufverstärkung
V0 = sum(V[:6]) / len(V[:6])
print(f'Zweite Leerlaufverstärkung V0 = {V0:.2f}')

x = np.log10(f_kHz[-5:])
y = np.log10(unumpy.nominal_values(V[-5:]))
V_err = unumpy.std_devs(V[-5:])
...
print(f'Zweite Grenzfrequenz f_g1 = {f_g_kHz:.2f} kHz')
# Bandbreitenprodukt
B = V0 * f_g_kHz
print(f'Zweite Bandbreite ist B1 = {B:.2f} kHz')
...
# Abfall
f_fit_kHz = np.logspace(np.log10(min(f_kHz[-5:])), np.log10(max(f_kHz)), 100)
f_fit_Hz = f_fit_kHz * 1000
V_fit = 10 ** (m * np.log10(f_fit_kHz) + b)
# f_fit_Hz = np.logspace(np.log10(min(f_Hz[-5:])), np.log10(max(f_Hz)), 100)
