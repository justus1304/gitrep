...
x = np.log10(f_kHz[-4:])
y = np.log10(unumpy.nominal_values(V[-4:]))
...
# Abfall
f_fit_kHz = np.logspace(np.log10(min(f_kHz[-4:])), np.log10(max(f_kHz)), 100)
f_fit_Hz = f_fit_kHz * 1000
V_fit = 10 ** (m * np.log10(f_fit_kHz) + b)
# f_fit_Hz = np.logspace(np.log10(min(f_Hz[-4:])), np.log10(max(f_Hz)), 100)
