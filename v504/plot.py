...
ax.plot(x2, y2, "x", markersize=5, label=r"$I_\mathrm{H} = 1{,}8\,$ A")

# KL_3
x3, y3 = np.loadtxt("Daten/KL_3.txt", unpack=True)
ax.plot(x3, y3, "x", markersize=5, label=r"$I_\mathrm{H} = 1{,}8\,$ A")

# KL_4
x4, y4 = np.loadtxt("Daten/KL_4.txt", unpack=True)
ax.plot(x4, y4, "x", markersize=5, label=r"$I_\mathrm{H} = 5{,}0\,$ A")

# KL_5
x5, y5 = np.loadtxt("Daten/KL_5.txt", unpack=True)
ax.plot(x5, y5, "x", markersize=5, label=r"$I_\mathrm{H} = 5{,}0\,$ A")

# Achsenbeschriftungen und Legende
ax.set_xlabel("Spannung U [V]")
ax.set_ylabel("Strom I [mA]")
ax.legend(framealpha=1)  # Legende mit Transparenz
