...
\label{sec:1}
Da für die Bestimmung der effektiven Masse und somit für die Auswertung der Messwerte die Kenntnis der Magnetfeldverteilung entlang der Probe notwendig ist, wurde diese zunächst mit einer Hall-Sonde vermessen. Die Messwerte wurden in \autoref{fig:magnetfeldverteilung} dargestellt und mit einem quadratischen Fit versehen, um die Verteilung zu beschreiben. Die gefundenen Koeffizienten des Fits sind in der Legende der Grafik angegeben. Es ist zu erkennen, dass die Messdaten gut durch den quadratischen Fit beschrieben werden, jedoch wurden die Werte bei jeweils $\qty{80}{mm}$ sowie $\qty{120}{mm}$ nicht berücksichtigt, da diese offensichtlich Ausreißer darstellen, da das Magnetfeld dort nahezu Null ist. Die Messwerte sind \autoref{tab:magnetfeldverteilung} zu entnehmen.
...
Die Bestimmung der effektiven Masse erfolgt durch die Auswertung der gemessenen Faraday-Rotationswinkel der freien Ladungsträger in Abhängigkeit von der Wellenlänge.Zuallererst werden Alle gemessenen Winkel durch die Dicke der jewailigen Probe geteilt und somit skaliert. Dann werden die skalierten Rotationswinkel der undotierten Probe von den beiden gemesenen Rotationswinkeln bei den $1.2\cdot10^{18} \text{cm}^{-3}$ und $2.8\cdot10^{18} \text{cm}^{-3}$ dotierten Proben subtrahiert, um die freien Faraday-Rotationswinkel zu erhalten. Anschließend wurden diese gegen das Quadrat der Wellenlänge aufgetragen und mit einem linearen Fit versehen, um die Steigung zu bestimmen. Die effektive Masse kann dann aus der Steigung des Fits berechnet werden.  

\indent

Die gemessenen Faraday-Winkel sind \autoref{tab:rotationswinkel1} sowie \autoref{tab:rotationswinkel2} zu entnehmen. Die Rotationswinkel der zwei Proben, sowie der undotierten Probe, sind die halbierten Differenzen der aufgenommenen 
...
\begin{table}[H]
    \centering
    \caption{Gemessene Rotationswinkel sowie freie Rotation für Probe 2.}
    ...
    \end{tabular}   
\end{table}
Hier ist die physikalisch saubere Formulierung für deine Konstanten in einer LaTeX-align-Umgebung.

Dabei habe ich die Konstanten genau mit den Werten aufgeführt, die du in deinem Python-Skript benutzt hast, und die Einheiten in \text{...} gesetzt, damit sie im Dokument aufrecht und professionell gedruckt werden.

Du kannst den folgenden Code direkt in dein Dokument einbauen:
...
Nun werden die freien Rotationswinkel gegen das Quadrat der Wellenlänge aufgetragen und mit einem linearen Fit versehen, um die Steigung zu bestimmen.
In der Gleichung 
\begin{equation*}
...
als Steigung der Ausgleichsgeraden identifiziert werden. Durch Umstellen lässt sich dann durch Einsetzen der Konstanten aus \autoref{eqn:1}, sowie der in \autoref{sec:1} berechnete maximale Magnetfeldstärke am Ort der Probe, auf die effektive Masse schließen.
\begin{equation}
    \label{eqn:2}
...
\end{figure}
Es wurde bei der ersten Probe der erste Messwert, sowie bei beiden Proben der dritte und vierte als Ausreißer nicht in die Regression mit einbezogen. Die Parameter der Ausgleichsrechnung ergeben sich zu 
\begin{align*}
    a_{1.2dot} &= \qty{6.8+-2.4e+12}{rad/m^2} \\
    a_{2.8dot} &= \qty{6+-5e+12}{rad/m^2}
\end{align*}

und dadurch wiederum eingesetzt in \autoref{eqn:2} die effektiven Massen
\begin{align}
