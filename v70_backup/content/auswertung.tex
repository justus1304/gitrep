...
\subsection{Berechnung des Saugvermögens mit der Evakuierungskurve}
Um das Saugvermögen mithilfe der Evakuierungskurve zu bestimmen wird \autoref{eqn:100} genutzt. Es wird für jeden gemessenen druck Druck $p$, 
sowie den Anfangsdruck $p_0$ und den Enddruck $p_E = \qty{0.02}{\milli\bar}$ 
\begin{equation}
  \label{eqn:100}
  -\frac{S_\text{eff}}{V} \cdot t = \log{\frac{p\left(t\right)-p_E}{p_0-p_E}}
\end{equation}
berechnet, um daraus durch umstelen auf das Saugvermögen der Pumpe schliesen zu können. 
der ln in abhängigkeit der zeit ist in \ref{fig:10} mit den Ausgleichsgeraden in drei verschiedenen Abschnitten ,gewählt als (0-150) , (150-290) ,(290-599), 
dargestellt. Auserdem sind Messwerte und der Logarithmuswert \autoref{tab:10} zu entnehmen.
\begin{figure}
  \label{fig:10}
  \centering
...
Multipliziert man nun noch mit dem Volumen, welches ... beträgt ergeben sich die 
Saugvermögen in den jevailigen Bereichen zu
\begin{align}
  S_\text{eff1}^\text{eva} =& \qty{4.9+-0.5}{m^3/s}\\
  S_\text{eff2}^\text{eva} =& \qty{1.71+-0.17}{m^3/s}\\
  S_\text{eff3}^\text{eva} =& \qty{0.70+-0.07}{m^3/s}
\end{align}
...
\label{tab:10}
\centering
\caption{Messdaten der Evakuierungskurve Drehschieberpumpe}
...
Die messwerte des Druckanstiegs nach abschiebern des Pmpenventiels sind in den Tabellen \autoref{tab:11},\autoref{tab:12},\autoref{tab:13},\autoref{tab:14} gegeben. 
Aus diesen Drücken wird durch 
\begin{equation}
  \label{eqn:20}
  S_\text{eff}^\text{leck} = \frac{V}{p_\text{g}} \cdot \frac{\delta p}{\delta t}
\end{equation}
und mithilfe der Steigungsparameter der Ausgleichsrechnungen bei den jewailigen Gleichgewichtsdrücken, welche dem Quotienten $\frac{\delta p}{\delta t}$
entsprechen, das Saugvermögen bestimmt. den Folgenden Abbildungen sind die messwerte geplotet, sowie 
die Ausgleichsgraden zu entnehmen. Es ist zu erkennen, dass einige erste Werte bei der Regression ausgeschlossen wurden, sofern sie stark von einem linearen verlauf abgewichen haben. Diese starken abweichungen am anfang liegen wohlmöglich an eingeschlossener 
Luft im Ventil, welche schlagartig beim öffnen frei wird.

\begin{figure}[H]
  \includegraphics{"drehschieberLeckrate1.pdf"}
\end{figure}
Der Steigungsparameter der Ausgleichsgeraden beträgt bei einem gleichgewichtsdruck von 
$p_\text{g} = \qty{0.502}{\milli\bar}$
\begin{equation}
...
\end{figure}
\begin{equation}
  m_{\qty{50.07}{\milli\bar}} = \qty{2.11171 +- 0.00216}{\milli\bar\per\second}
\end{equation}
...
Aus diesen Steigungen der Ausgleichsrechnungen berechnen wir nun mit \autoref{eqn:20}
die Saugvermögen zu 
\begin{align}
  S_\text{eff0.5mbar}^\text{leck} =& \qty{0.68+-0.07}{\meter\cubed\per\second}\\
  S_\text{eff10mbar}^\text{leck} =& \qty{3.6+-0.4}{\meter\cubed\per\second}\\
  S_\text{eff50mbar}^\text{leck} =& \qty{5.1+-0.5}{\meter\cubed\per\second}\\
  S_\text{eff105mbar}^\text{leck} =& \qty{4.5+-0.5}{\meter\cubed\per\second}
\end{align}
...
\begin{table}[H]
  \caption{Messwerte für die Leckratenmethode der Drehschieberpumpe bei $p_\text{g} = \qty{100}{\milli\bar}$.2 Messungen für mittelwertsbildung.}
\label{tab:14} 
...
\label{sec:100}
Die Berechnung folgt analog zu der berechnung bei der Drehschieberpumpe.
 \autoref{tab:messung1},\autoref{tab:messung2},\autoref{tab:messung3},\autoref{tab:messung4} gegeben. 
 Es folgen die geplotteten Mesdaten mit den berechneten Ausgleichsgeraden.

\begin{figure}[H]
  \includegraphics{"drehschieberLeckrate1.pdf"}
\end{figure}
Der Steigungsparameter der Ausgleichsgeraden beträgt bei einem gleichgewichtsdruck von 
\qty{5.04e-5}{\milli\bar}
\begin{equation}
...
\begin{table}[H]
    \centering
    \caption{Messdaten des Druckanstiegs zur Bestimmung der Leckrate $p_g = \qty{5.04e-5}{\milli\bar}$.}
    \label{tab:messung1}
...
\begin{table}[H]
    \centering
    \caption{Messdaten des Druckanstiegs zur Bestimmung der Leckrate $p_g = \qty{56.96e-5}{\milli\bar}$.}
    \label{tab:messung2}
    \begin{tabular}{S[table-format=3.0] S[table-format=1.2e-1] @{ \pm } S[table-format=1.2e-1]}
        \toprule
        {Zeit $t / \si{\second}$} & \multicolumn{2}{c}{Druck $p / \si{\milli\bar}$} \\
...
\begin{table}[H]
    \centering
    \caption{Messdaten des Druckanstiegs zur Bestimmung der Leckrate $p_g = \qty{1.03e-4}{\milli\bar}$.}
    \label{tab:messung3}
    \begin{tabular}{S[table-format=3.0] S[table-format=1.2e-1] @{ \pm } S[table-format=1.2e-1]}
        \toprule
        {Zeit $t / \si{\second}$} & \multicolumn{2}{c}{Druck $p / \si{\milli\bar}$} \\
...
\begin{table}[H]
    \centering
    \caption{Messdaten des Druckanstiegs zur Bestimmung der Leckrate $p_g = \qty{1.91e-4}{\milli\bar}$.}
    \label{tab:messung4}
    \begin{tabular}{S[table-format=3.0] S[table-format=1.2e-1] @{ \pm } S[table-format=1.2e-1]}
        \toprule
        {Zeit $t / \si{\second}$} & \multicolumn{2}{c}{Druck $p / \si{\milli\bar}$} \\
...
Volumen der Aperatur $V = \qty{33+-3.3}{\liter}$.

Die gemittelten Messwerte, sowie derren Fehler sind \autoref{tab:100} zu entnehmen. Dabei ergibt sich der Fehler aus dem Fehler des Mittelwerts 
und dem Systematischen Fehler. Die geploteten Messwerte inklusive Ausgleichsgeraden befinden sich in 
\autoref{fig:100}. Dabei wurden für die Ausgleichsrechnung die ersten 12 Messwerte berücksichtigt, da 
dort ein linearer Abfall erkennbar ist. Danach nähert sich der Druck dem Enddruck an und es wird kaum noch gepumpt.

\begin{figure}
  \label{fig:100}
  \centering
  \includegraphics{/home/justus/gitrep/v70/build/turboevakuierung.pdf}
\end{figure}
Der steigungsparameter der ausgleichsrechnung beträgt 
\begin{equation}
  m = \qty{-46.5(1.4)e-2}{}
\end{equation}
und damit ergibt sich ein Saugvermögen von
\begin{equation}
  S_\text{eff}^\text{Eva} = \qty{15.4+-1.6}{l/s}.
\end{equation}
...
Als letztes wird zwische den Beiden Pirani- Druckmessgeräten eine 
erheblichere Engstelle eingebaut, sodass sich sofort eine Größere Druckdifferenz zwischen 
den Sensoren einstellt, welche aus dem Geringeren Leitwert der Engstelle resultiert.
Nun werden mit beiden Messgeräten analog zu \autoref{sec:100} die Saugvermögen 
berechnet um den Unterschied zu verdeutlichen. Die Messwerte und Regression für 
den Drucksensor vor der Engstelle, also zwischen Pumpe und Engstelle befinden sich in 
\autoref{fig:101}, sowie die für das messgerät nach der Engstelle in \autoref{fig:102}.
Die Messwerte befinden sich in \autoref{tab:105}.
...
\begin{align}
m_{\text{vor}}    &= \qty{1.99(0.01)e-04}{} \\
m_{\text{hinter}} &= \qty{1.84(0.009)e-04}{}
\end{align}

und daraus Ergeben sich die Saugvermögen wiederum zu 
Die Parameter der ausgleichsrechnungen ergeben sich zu 
\begin{align}
S_\text{vor} =& \qty{35.1(3.5)}{l/s}\\
S_\text{hinter} =& \qty{2.68(1.0)}{l/s}
\end{align}
...
Als zusammenfassung aller berechneter Saugvermögen sollen diese hier einmal in abhängigkeit des 
Drucks zusammengefasst werden. In \autoref{fig:103} befinden sich jene Saugvermögen der Drehschieberpumpe und in 
\autoref{fig:104} jene der Turbomolekularpumpe.

\begin{figure}[h]
  \caption{Saugvermögen der Drehschieberpumpe in anbhängigkeit des Drucks}
  \label{fig:103}
\includegraphics{build/saugvermögenDrehschieberpumpe.pdf}
\end{figure}

\begin{figure}[h]
  \caption{Saugvermögen der Turbomlekularpumpe in abhängigkeit des Drucks}
  \label{fig:104}
...
Da sowohl der gemessene druck $p$ als auch der Enddruck $p_E$ fehlerbehaftet sind ($\Delta p$ und $\Delta p_E$), berechnet sich der Fehler $\Delta y$ über die Gaußsche Fehlerfortpflanzung
\begin{equation}
...
Setzt man diese in die fehlerfortpflanzung ein, erhält man für den Fehler des Logarithmus:
\begin{equation}
