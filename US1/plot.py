...
def acryl():
    ...
    ax.set_ylabel(r"Länge / $\unit{\meter}$")
    ax.set_xlabel(r"Laufzeit / $\unit{\second}$")
...
def acryl2():
    ...
    ax.set_ylabel(r"$\ln(\frac{U_2}{U_1})$")
    ax.set_xlabel(r"Länge / $\unit{\meter}$")
    #ax.set(xlabel='$t/ \unit{\second}$',ylabel='$h/ \unit{\meter}$')
    ax.legend()
    ax.grid(True)
    #ax.axvline(x = 560,color = 'red', linestyle='--')
...
def fuellstand():
    ...
    ax.set_ylabel(r"Füllstand / $\unit{\micro\second}$")
    ax.set_xlabel(r"Laufzeitunterschied / $\unit{\milli\liter}$")
    ax.grid(True)
    ax.legend()
