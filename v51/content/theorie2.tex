...
Der Operationsverstärker, kurz OPV, ist ein gleichspannungsgekoppelter Differenzverstärker. Er kann je nach Anschluss in vielen Bereichen Verwendung finden. So kann er als Integrierer oder Differenzierer einer Spannung genutzt werden. Daher erhält der OPV auch seinen Namen. Er kann jedoch auch zur reinen Verstärkung eines Eingangssignals genutzt werden oder als eine Art Schaltvorrichtung in Form einer Schmitt-Trigger-Schaltung. 
...
Der Strom $I_1$ durch den Eingangswiderstand ergibt sich aus der Spannungsdifferenz zwischen der Eingangsspannung $U_e$ und dem Potenzial des virtuellen Nullpunkts $U_-$:
\begin{equation*}
    I_1 = \frac{U_e - U_-}{R_1} = \frac{U_e}{R_1}
\end{equation*}
Analog gilt für den Strom $I_2$ durch den Rückkopplungswiderstand $R_2$ mit der Ausgangsspannung $U_a$:
\begin{equation*}
    I_2 = \frac{U_a - U_-}{R_2} = \frac{U_a}{R_2}
\end{equation*}
Setzt man diese Ströme in die Knotenregel $I_1 = -I_2$ ein, folgt:
\begin{equation*}
    \frac{U_e}{R_1} = -\frac{U_a}{R_2}
\end{equation*}
Durch Umstellen nach dem Verhältnis von Ausgangs- zu Eingangsspannung erhält man den Verstärkungsfaktor $v$:
\begin{equation}
...
\end{equation*}
Durch Integration nach der Zeit über das Intervall $[0, t]$ erhält man die Ausgangsspannung des Umkehr-Integrators:
\begin{equation}
    U_a(t) = -\frac{1}{RC} \int_{0}^{t} U_e(t') \, dt'
\end{equation}
Betrachtet man das Verhalten im Frequenzraum für eine harmonische Wechselspannung $U(t) = U_0 \cdot e^{i\omega t}$, so führt die Integration auf einen Faktor $\frac{1}{i\omega}$ \cite{tietzeschenk}. Für das Amplitudenverhältnis ergibt sich folglich eine Antiproportionalität zur Kreisfrequenz bzw. Frequenz $f$:
\begin{equation}
    \frac{U_a}{U_e} \propto \frac{1}{f}
\end{equation}
...
\end{equation*}
Durch Multiplikation mit $-R$ erhält man die finale Gleichung für die Ausgangsspannung des Differenzierers:
\begin{equation}
...
\end{equation}
Setzt man die Bedingung für das Umschalten ($U_+ = 0$) an und substituiert $U_a$ durch die Sättigungsspannung $\pm U_S$, erhält man durch Umstellen nach der Eingangsspannung die Schaltschwellen bzw. Grenzspannungen $U_{\text{kipp}}$:
\begin{equation}
