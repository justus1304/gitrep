...
\label{BB84}
In diesem Abschnitt wird sich erst einmal mit der Funktionsweise der Quantenkryptographie vertraut gemacht. 
...
Anschließend wird mithilfe des Schlüssels von Alice eine Nachricht verschlüsselt. Die Nachricht lautet: "ALSO". Diese Nachricht wird in Binärcode umgewandelt und mithilfe des ersten 20 Bits des Schlüssels von Alice verschlüsselt.
Beim Verschlüsseln wird der Schlüssel über die Nachricht gelegt und in der verschlüsselten Nachricht wird bei gleichen Zahlen, also (1 und 1) oder (0 und 0), eine Null als verschlüsseltes Bit vermerkt und bei ungleichen Bits (1 und 0) oder (0 und 1) eine 1 als verschlüsseltes Bit vermerkt. Das Prinzip ist das gleiche wie bei einem XOR-Gatter in der Digitaltechnik.
\begin{align*}
...
Die gesendeten und empfangenen Bits, welche durch eine gleiche Basis von Alice und Bob ausgewählt wurden, sind \autoref{tab:vergleichEve} zu entnehmen. Die fehlerhaften Bits wurden mit einem $x$ markiert und belaufen sich auf acht Fehler bei 
32 ausgewählten verglichenen Bits. Das führt zu einer Fehlerrate von 
\begin{equation}
...
In diesem Abschnitt wird die Schlüsselgenerierung mit Decoy States beschrieben. Es werden statt Bits jetzt Trits gesendet (0, 1 oder 2) und das wieder entweder in der $\times$ Basis oder in der $+$ Basis.
Die Codierungen sind wie folgt in \autoref{tab:ternary_protocol} dargestellt. 
...
\begin{table}[htbp]
\centering
\caption{Codierung der Trits in den beiden Basen und den Decoy States}
...
Diese Umwandlung erfolgt quasi analog zu Alices Codierung der Trits, nur müssen jetzt entweder die roten oder 
grünen Bits in der jeweiligen Phase interpretiert werden. Weiteres ist 
...
Nun werden 52 Trits in zufälligen Basen verschickt und es wird wie in \autoref{BB84} verfahren.

Nach Verwerfen der Trits, bei denen Alice und Bob unterschiedliche Basen gewählt haben, bleiben 32 Trits übrig.
Diese werden in \autoref{eqn:trits} verglichen.
...
zu einem Anteil an Decoy States von $\frac{{N_{decoy}}}{{N_{shifted Bits}}} = 0$ führt. Diese wird im folgenden \autoref{sec:eve} mit der Fehlerrate nach Dazuschalten 
von Eve verglichen.

\subsection{Abhörversuch von Eve bei der Schlüsselgenerierung mit Decoy States}
In diesem Abschnitt wird der Abhörversuch von Eve bei der Schlüsselgenerierung mit Decoy States beschrieben. Es werden wieder 52 Trits von Alice an Bob gesendet, aber diesmal mit Decoy States. 
Eve versucht wieder die Nachricht von Alice abzufangen und zu entschlüsseln. Wenn sie die falsche Basis wählt 
und somit falsche Trits an Bob weiterleitet, führt das zu einer Fehlerrate von $\frac{N_\text{Error}}{N_\text{shifted Trits}} > 0$ und zur Entstehung von Decoy States, welche von Alice und Bob als Indiz für Eves Abhörversuch genutzt werden können.
Erhält Bob also so einen Decoy State bei gleicher Basiswahl wie Alice, ist Eve aufgedeckt und der Schlüssel sollte von Alice und Bob nicht verwendet werden. Die gesendeten sowie empfangenen Trits 
sind in \autoref{tab:vergleichEveDecoy} dargestellt. Die fehlerhaften Trits wurden mit einem $x$ markiert und die Decoy States als Decoy betitelt. 
