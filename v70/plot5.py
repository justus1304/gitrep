...
# CSV einlesen
df1 = pd.read_csv('/home/justus/gitrep/v70/Daten/Letzte/TPG_all_data_2026_04_20-PM05_41_21.csv',sep=',')
...
def tabelle(zeit, druck, fehler,druck20,fehler20,pfad):
    df = pd.DataFrame({
    'Zeit': zeit,
    'Druck': druck,
    'Fehler': fehler,
...
tabelle(zeit1,druck1,fehler1,druck2,fehler2,"build/letztetab.csv")
...
print(saug(V,1.930*10**(-4),param1))
print(saug(V,2.34*10**(-3),param2))
#print(saug(V,1.03*10**(-4),param3))
#print(saug(V,1.91*10**(-4),param4))

print('saugvermögen m³/s')
print(saug(V,1.930*10**(-4),param1)*3.6)
print(saug(V,2.34*10**(-3),param2)*3.6)
#print(saug(V,1.03*10**(-4),param3)*3.6)
#print(saug(V,1.91*10**(-4),param4)*3.6)

#druck = [5.04*10**(-5),6.96*10**(-5),1.03*10**(-4),1.91*10**(-4)]
#saug = [saug(V,5.04*10**(-5),param1)*3.6,saug(V,6.96*10**(-5),param2)*3.6,saug(V,1.03*10**(-4),param3)*3.6,saug(V,1.91*10**(-4),param4)*3.6]
#saug1 = []
...
#
#fig, ax = plt.subplots()
#ax.errorbar(druck, saug1,linestyle='none',errorevery=20, yerr=saug1f, fmt='o', markersize=0.4, color='black', linewidth=0.5,capsize=3, label='Messwerte')
##x = np.linspace(0,210,1000)
##ax.plot(x,m_fit1 * x + b_fit1,label='Regression')
#ax.set_xlabel('p / mbar')
#ax.set_ylabel('S / l/sec')
#fig.suptitle('Saugvermögen in abhängiigkeit des Drucks')
#ax.legend()
#fig.savefig('build/saugvermögenDrehschieb.pdf')